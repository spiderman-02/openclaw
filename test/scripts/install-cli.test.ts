...
HOOK
  ...
  it.each([
    ...
    ({ existing, starts }) => {
      ...
      const install = () =>
        runInstallCliShell(
          [
            ...
            "download_file() {",
            '  case "$1" in',
            "    */SHASUMS256.txt) printf 'fixture-sha  node-v24.15.0-linux-x64.tar.gz\\n' > \"$2\" ;;",
            "    *) printf 'node tarball fixture\\n' > \"$2\" ;;",
            "  esac",
            "}",
            "sha256_file() { printf 'fixture-sha\\n'; }",
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain(
        "Installed Node 24.15.0 must provide Node >= 22.22.3 with WAL-reset-safe SQLite",
      );
      expect(result.stdout).toContain(
        starts
      ...
      expect(readlinkSync(activeNode)).toBe(join(prefix, "tools", "node-v24.15.0"));
